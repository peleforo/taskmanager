			...
			<div class="col-xs-12 col-sm-11 col-md-11 col-lg-11"  >
				...
				<div class="col-xs-12 col-sm-11 col-md-11 col-lg-11">	
						...
						<ul class="tabs" style="font-size: 30px;">
							<li class="active" ><a href="#configtv">configuration tv
								<!--AFFICHAGE DU NOMBRE DE DOSSIER-->
								<?php
											     
										$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND spec = 1");
						                $nbredossier = $reqetat->rowcount();

							            echo '<span>' . $nbredossier .  '</span>' ;
								?>
							</a></li>
							<li><a href="#specpara">speclation parabole
								<!--AFFICHAGE DU NOMBRE DE DOSSIER-->
								<?php
											     
										$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND spec = 0 ");
						                $nbredossier = $reqetat->rowcount();
						                $req = $reqetat->fetch();
						...
						<ul class="tabs" style="font-size: 30px;">
							<li class="active"><a href="#encours">En cours
								...
								<?php
										...
										if ($req['spec'] = 0) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 1 AND spec = 0 ");
							                $nbredossier = $reqetat->rowcount();
										...
										elseif ($req['spec'] = 1) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 1 AND spec = 1 ");
							                $nbredossier = $reqetat->rowcount();
							...
							<li><a href="#enattente">En attente
								...
								<?php
										...
										if ($req['spec'] = 0) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 2 AND spec = 0 ");
							                $nbredossier = $reqetat->rowcount();

								            echo '<span>' . $nbredossier .  '</span>' ;
										}
										elseif ($req['spec'] = 1) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 2 AND spec = 1 ");
							                $nbredossier = $reqetat->rowcount();
							...
							<li><a href="#reporte">Reporté
								...
								<?php
										...
										if ($req['spec'] = 0) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 3 AND spec = 0 ");
							                $nbredossier = $reqetat->rowcount();

								            echo '<span>' . $nbredossier .  '</span>' ;
										}
										elseif ($req['spec'] = 1) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 3 AND spec = 1 ");
							                $nbredossier = $reqetat->rowcount();
							...
							<li><a href="#valide">Validé
								...
								<?php
										...
										if ($req['spec'] = 0) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 4 AND spec = 0 ");
							                $nbredossier = $reqetat->rowcount();

								            echo '<span>' . $nbredossier .  '</span>' ;
										}
										elseif ($req['spec'] = 1) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 4 AND spec = 1 ");
							                $nbredossier = $reqetat->rowcount();
							...
							<li><a href="#perdu">Perdu
								...
								<?php
										...
										if ($req['spec'] = 0) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 5 AND spec = 0 ");
							                $nbredossier = $reqetat->rowcount();

								            echo '<span>' . $nbredossier .  '</span>' ;
										}
										elseif ($req['spec'] = 1) {
											$reqetat = $bdd->query("SELECT * FROM dossier WHERE projetconcerne = 2 AND statutdossier = 5 AND spec = 1 ");
							                $nbredossier = $reqetat->rowcount();
